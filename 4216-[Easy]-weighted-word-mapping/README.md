...
## 🧠 Approach
- The given solution uses a StringBuilder to efficiently build the result string.
- It iterates over each word in the input array, and for each word, it calculates the weighted sum of its characters.
- The weighted sum is then taken modulo 26 to ensure the result is within the range of lowercase letters.
- The result is appended to the StringBuilder as a single character.

## ⏱️ Complexity
- Time: O(n*m), where n is the number of words and m is the maximum length of a word.
- Space: O(n), where n is the number of words, as we need to store the result string.
