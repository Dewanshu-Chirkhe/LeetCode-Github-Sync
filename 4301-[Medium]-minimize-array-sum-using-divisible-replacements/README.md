...
Example 1:
  Input:  nums = [3,6,2]
  Output: 7
  Explanation: Choose a = 1, b = 2, where nums[a] = 6 and nums[b] = 2. Since 6 % 2 == 0, replace nums[1] with nums[2].
...
Example 2:
  Input:  nums = [4,2,8,3]
  Output: 9
  Explanation: Choose a = 0, b = 1, where nums[a] = 4 and nums[b] = 2. Since 4 % 2 == 0, replace nums[0] with nums[1].
...
Example 3:
  Input:  nums = [7,5,9]
  Output: 21
  Explanation: There is no pair (a, b) such that nums[a] % nums[b] == 0.
...
## 🧠 Approach
- The problem is solved by iterating through each element in the array and finding the smallest divisor that is also present in the set of unique elements.
- For each element, we check all possible divisors from 1 to the square root of the element.
- If the element is divisible by a divisor, we check if the divisor and its corresponding quotient are present in the set of unique elements.
- We keep track of the smallest divisor found and add it to the total sum.
