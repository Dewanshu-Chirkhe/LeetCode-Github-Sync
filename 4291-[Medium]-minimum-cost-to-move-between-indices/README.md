...
Example 2:
  ...
  Output: [4,1,3]
  Explanation: The closest indices are [1, 2, 1, 2] respectively.
...
## 🧠 Approach
- The problem is solved by first calculating the closest index to the current index in the array, which is used to determine the cost of moving between indices.
- Two arrays, `forward` and `backward`, are used to store the cumulative cost of moving to the right and left respectively.
- The `calculateCost` function is used to calculate the cost of moving between two indices based on the `forward` and `backward` arrays.
- The `minCost` function iterates over the queries and uses the `calculateCost` function to calculate the cost of moving between the given indices.
