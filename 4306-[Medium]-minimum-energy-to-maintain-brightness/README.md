...
## 🧠 Approach
- Sort the intervals based on their start time to process them in order.
- Initialize variables to keep track of the active time and the current start and end times of the active intervals.
- Iterate through the intervals, merging any overlapping intervals and updating the active time.
- Calculate the total energy required by multiplying the active time by the number of bulbs needed to maintain the brightness.

## ⏱️ Complexity
- Time: O(n log n) due to the sorting of intervals.
- Space: O(1) as we only use a constant amount of space to store the variables.
