...
## 🧠 Approach
- The solution iterates over each element in the input array `nums`.
- For each element, it calculates the parity (0 or 1) of the element by taking the modulus with 2.
- Then, it counts the number of elements with opposite parity by iterating from the current index to the end of the array and checking if the parity of each element is different from the current element's parity.
- The count of elements with opposite parity is stored in the `ans` array at the current index.
