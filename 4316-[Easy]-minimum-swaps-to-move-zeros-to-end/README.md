...
## 🧠 Approach
- Initialize a counter `zeroCount` to count the total number of zeros in the array.
- Iterate through the array to count the total number of zeros.
- Initialize two pointers, `zeroEnd` and `idx`, to count the number of zeros at the end of the array.
- Move the `idx` pointer from the end of the array towards the beginning, decrementing `temp` until it reaches zero.
